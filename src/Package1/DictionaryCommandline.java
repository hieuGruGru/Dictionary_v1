...
public class DictionaryCommandline {
    ...
    public static void dictionarySearcher(Dictionary dictionary1) { //Liệt kê các từ có phần đầu trùng với chuỗi nhập vào
        ...
        for (int index = 0; index < dictionary1.arrayOfWord.size(); index++) {
            ...
            if (word_target.toLowerCase().indexOf(prefixString.toLowerCase()) == 0) {//So sánh xem prefixString có phải là
                                                                                     //phần đầu của một từ trong từ điển không
                showOneWords(dictionary1, index);
            }
        }
    }

    public static void dictionaryBasic(Dictionary dictionary1) throws IOException { //Từ điển đơn giản, có chức năng thêm
                                                                                    //từ mới từ commandline và hiển thị tất cả từ điển
    ...
    public static void dictionaryAdvenced(Dictionary dictionary1) throws IOException { //Từ điển nâng cao, có chức năng nạp từ từ file .txt,
                                                                                       //tìm kiếm từ và hiển thị tất cả từ điển
        DictionaryManagement.insertFromFile(dictionary1);
        DictionaryCommandline.showAllWords(dictionary1);
        DictionaryManagement.dictionaryLookup(dictionary1);
    }

    public static String writeSpace(StringBuilder separateString, int n) { //Tạo chuỗi khoảng trắng với độ dài nhất định,
                                                                           //nhằm căn dòng khi hiển thị từ điển
        for (int i = 0; i < n; i++) {
